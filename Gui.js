(async () => {
    // --- Helper Functions for React & Webpack Stability ---

    // Robust React Node Finder
    // Fixes "owner" undefined errors by safely searching for the game state
    const getStateNode = () => {
        try {
            const appNode = document.querySelector("#app > div > div");
            if (!appNode) return null;

            // Find the internal React Fiber key
            const reactKey = Object.keys(appNode).find(key => 
                key.startsWith("__reactFiber") || key.startsWith("__reactInternalInstance")
            );
            if (!reactKey) return null;

            let fiber = appNode[reactKey];

            // 1. Try the specific path many cheats rely on
            if (fiber.child && fiber.child._owner && fiber.child._owner.stateNode) {
                return fiber.child._owner.stateNode;
            }

            // 2. Fallback: Search up the tree for the game controller
            let curr = fiber;
            while (curr) {
                if (curr.stateNode && (curr.stateNode.props?.liveGameController || curr.stateNode.state?.question)) {
                    return curr.stateNode;
                }
                curr = curr.return;
            }
        } catch (e) {
            console.warn("ZipGUI: Could not find stateNode", e);
        }
        return null;
    };

    // Robust Webpack Access
    // Handles updates where Blooket changes webpack global names
    const getWebpack = () => {
        if (window.webpackChunkblooket_web) return window.webpackChunkblooket_web;
        if (window.webpackJsonp) return window.webpackJsonp;
        return [];
    };
    ...
    let C = {
        global: [{
            ...
            run: async function(e, t, a) {
                ...
                let r = getStateNode();
                if(!r) return;
                let i = Array.from(document.querySelectorAll("[class*='packsWrapper'] > div")).reduce((e, t) => (t.querySelector("[class*='blookContainer'] > img") || (e[t.querySelector("[class*='packImgContainer'] > img").alt] = parseInt(t.querySelector("[class*='packBottom']").textContent)), e), {}),
            ...
            run: (function(newname) {
                (async () => {
                    const reactHandler = getStateNode;
                    let i = document.createElement('iframe');
                    document.body.append(i);
            ...
            run: function(e) {
                ...
                document.head.appendChild(t),
                    ...
                    window.prompt = t.contentWindow.prompt.bind(window),
                    t.remove();
                    // Fix: accessed liveGameController safely
                    a().props.liveGameController.setVal({
                        path: "c/" + a().props.client.name + "/bg",
                        val: e
                    })
            ...
            run: function() {
                    ...
                    n.addEventListener("keydown", function(a) {
                        13 === a.keyCode && (function a(o) {
                                ...
                                if (h)
                                    switch (h.cmd) {
                                        case "cb":
                                            r = h.args.join(" "),
                                                ...
                                                ]).webpack("MDrD").a)[r = Object.keys(n).find(e => r.toLocaleLowerCase() === e.toLocaleLowerCase())] ? (s("Setting blook to " + r + "!"), e().props.liveGameController.setVal({
                                                    id: e().props.client.hostId,
                                                    path: "c/" + e().props.client.name,
                                                    val: {
                                                        b: r
                                                    }
                                                }), e().props.client.blook = r) : s("No blook with that name was found!");
                                            break;
                                        case "clear":
                                            i.innerHTML = "";
                                            break;
                                        case "dumpstate":
                                            Object.keys(e().state).map(t => {
                                                var a = e().state[t];
                                                if (null == a)
                                                    return "N/A";
                                            ...
                                            break;
                                        case "list":
                                            e().props.liveGameController.getDatabaseVal("c").then(e => {
                                        ...
                                        case "setval":
                                            l = h.args,
                                                e().props.liveGameController.setVal({
                                                    path: "c/" + e().props.client.name + "/" + l[0],
                                        ...
                                        case "dump":
                                            ! function t(a) {
                                                e().props.liveGameController.getDatabaseVal("c/" + a).then(e => {
                                        ...
                                        case "unlock":
                                            p = h.args.join(" "),
                                                ...
                                                ]).webpack("MDrD").a)[p = Object.keys(u).find(e => p.toLocaleLowerCase() === e.toLocaleLowerCase())] ? (e().state.unlocks.push(p), e().forceUpdate()) : s("No blook with that name was found!");
                                            break;
                                        case "code":
                                            s("Game Code: " + e().props.client.hostId);
                                            break;
                                        default:
                                    ...
                                    e().props.liveGameController.setVal({
                                        id: e().props.client.hostId,
                                        path: "c/" + e().props.client.name + "/msg",
                                        val: {
                                            i: t,
                                            msg: o
                                        }
                ...
                var l = e().props.liveGameController._liveApp.database()._delegate._repoInternal.server_.onDataUpdate_;

                function c(e) {
                    window.logsv && s("Path: " + e.path.split("/").splice(2, 2).join("/") + " Val: " + ("object" == typeof e.val ? JSON.stringify(e.val) : e.val))
                }
                e().props.liveGameController._liveApp.database()._delegate._repoInternal.server_.onDataUpdate_ = function(e, t, a, o) {
                        var r,
                            i;
                        console.log(e, t, a, o),
                            r = e,
                    ...
                    window.logsv = !1;
                var d = e().props.liveGameController.setVal;
                e().props.liveGameController.setVal = function() {
                        c.apply(this, arguments),
                            d.apply(this, arguments)
                    },
                    e().props.liveGameController._liveApp.database().ref(`${e().props.client.hostId}`).on("value", e => {}),
            ...
            run: function() {
                ...
                a.get("https://dashboard.blooket.com/api/users").then(async ({
                    ...
                    if (e = Object.entries(e).filter(([e, t]) => 1 < t && !["Legendary", "Chroma", "Mystical"].includes(getWebpack().push([
                            [], {
                                1234(e, t, a) {
                                    t.webpack = a
                                }
        ...
        voyage: [{
            ...
            run: function() {
                ...
                function setv(args) {
                    reactHandler().props.liveGameController.setVal({
                        path: "c/" + reactHandler().props.client.name + "/" + args[0],
            ...
            run: function(userInput) {
                ...
                function setValForPlayer() {
                    getReactOwner().props.liveGameController.getDatabaseVal("c/").then(data => {
                        if (data != null) {
                ...
                function setv(args) {
                    getReactOwner().props.liveGameController.setVal({
                        path: "c/" + getReactOwner().props.client.name + "/" + args[0],
                        val: args.slice(1, args.length).join(" ")
                    });
                }
        ...
        brawl: [{
            ...
            run: function() {
                for (let e of getStateNode().game.current.scene.scenes[0].physics.world.colliders._active.filter(e => e.callbackContext?.toString().includes("invulnerableTime"))) {
                    var t = e.object2;
            ...
            run: function() {
                getStateNode().game.current.scene.scenes[0].physics.world.colliders._active.find(e => e.collideCallback?.toString().includes("magnetTime")).collideCallback({
                    active: !0
                }, {
                    active: !0,
                    setActive() {},
            ...
            run: function() {
                getStateNode().game.current.scene.scenes[0].physics.world.bodies.entries.forEach(e => {
                    try {
            ...
            run: function() {
                getStateNode().game.current.scene.scenes[0].physics.world.bodies.entries.forEach(e => e?.gameObject?.receiveDamage?.(e.gameObject.hp, 1))
            }
        }, {
            name: "Reset Health",
            description: "Resets health and gives invincibility for 3 seconds",
            run: function() {
                getStateNode().game.current.scene.scenes[0].game.events._events.respawn.fn()
            }
        }, {
        ...
        cafe: [{
            ...
            run: function() {
                ...
                document.body.append(e),
                    window.alert = e.contentWindow.alert.bind(window),
                    e.remove(),
                    "/cafe/shop" !== window.location.pathname ? alert("This can only be run in the shop") : (e = getStateNode()).setState({
            ...
            run: function(name) {
                if (this.enabled) {
                    ...
                    function spamAction() {
                        ...
                        if (handler && handler.props && handler.props.liveGameController) {
                            handler.props.liveGameController.setVal({
                                id: handler.props.client.hostId,
                                path: "c/" + handler.props.client.name + "/tat",
                                val: action
                            });
                        }
                    }
            ...
            run: (function(targetPlayer) {
                ...
                reactHandler().props.liveGameController.setVal({
                    id: reactHandler().props.client.hostId,
                    path: "c/" + reactHandler().props.client.name + "/tat",
            ...
            run: function() {
                ...
                document.body.append(e),
                    window.alert = e.contentWindow.alert.bind(window),
                    e.remove(),
                    "/cafe" !== window.location.pathname ? alert("This can't be run in the shop") : (e = getStateNode()).setState({
        ...
        crypto: [{
            ...
            data: null,
            run: function() {
                this.enabled ? (this.enabled = !1, clearInterval(this.data), this.data = null) : (this.enabled = !0, this.data = setInterval(() => getStateNode().setState({
                    choices: [{
                        type: "mult",
                        val: 3,
                        rate: .075,
        ...
        defense: [{
            ...
            run: function() {
                for (var i = 0; i < 10; i++) {
                    getStateNode().tiles[i] = Array(10).fill(0);
                }
            }
        }, {
            name: "Remove Enemies",
        ...
        defense2: [{
            ...
            description: "Makes all placed towers overpowered",
            run: function() {
                getStateNode().state.towers.forEach(e => {
        ...
        doom: [{
            ...
            run: function() {
                ...
                document.body.append(e),
                    window.alert = e.contentWindow.alert.bind(window),
                    e.remove(),
                    "/tower/map" == window.location.pathname ? (e = getStateNode()).props.tower.cards.forEach(e => {
                        e.strength = 20,
                            e.charisma = 20,
                            e.wisdom = 20
                    }) : alert("You need to be on the map to run this cheat!")
            ...
            run: function() {
                ...
                document.body.append(e),
                    window.alert = e.contentWindow.alert.bind(window),
                    e.remove(),
                    "/tower/battle" == window.location.pathname ? getStateNode().setState({
                        myHealth: 100
            ...
            run: function() {
                var e = document.createElement("iframe"),
                    e = (document.body.append(e), window.alert = e.contentWindow.alert.bind(window), e.remove(), getStateNode());
        ...
        factory: [{
            ...
            run: function() {
                getStateNode().state.blooks.forEach(e => e.level = 4)
            }
        }, {
        ...
        fishing: [{
            ...
            enabled: !1,
            run: function() {
                const componentInstance = getStateNode();
                if (this.enabled) {
                    this.enabled = !1;
            ...
            run: function(e) {
                var t = document.createElement("iframe"),
                    t = (document.body.append(t), window.alert = t.contentWindow.alert.bind(window), t.remove(), getStateNode());
                t.safe = !0,
                    t.props.liveGameController.setVal({
                        path: "c/" + t.props.client.name,
                        val: {
